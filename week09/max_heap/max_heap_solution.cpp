...
void MaxHeap::maxHeapify(int i) {
  ...
  if (r < count && element[r] > elements[largest]) {
    largest = r;
  }
...
int MaxHeap::maximum() {
  // TODO
  if (count == 0) {
    cerr << "MAXIMUM: heap is empty" << endl;
    exit(1);
  }
  else {
    return elements[0];
...
int MaxHeap::deleteMaximum() {
  // TODO
  if (count == 0) {
    cerr << "DELETE: heap is empty" << endl;
    exit(1);
  }
